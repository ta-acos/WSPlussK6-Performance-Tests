...
const MAX_BODY_BYTES = parseInt(__ENV.ERROR_SAMPLE_BODY_BYTES || '500', 10);
...
export function recordError(res, ctx = {}) {
  try {
    ...
    console.log(`[ERROR_RECORD]${JSON.stringify({
      timestamp,
      endpoint: endpointTag,
      method,
      status,
  ...
  try {
    ...
    samples.push({
      ...
      '❌ Issue Description': {
        ...
        'Impact': 'These requests returned HTTP 200 (success) but took longer than expected, indicating potential performance issues',
      ...
      '📋 Individual Error Details': `Each of the ${count} errors includes:\n` +
      ...
      '📍 Where to Find Full Details': {
        ...
        'Each Error Block Contains': 'Timestamp, Method, Endpoint, Status, Type, Full Message, JP ID, Duration, Response Body',
      ...
      '🎯 For Stakeholders': {
        ...
        'Performance Impact': rate > 0.1 ? '⚠️ High error rate - investigate server performance' : rate > 0.05 ? '⚠️ Moderate error rate - monitor closely' : '✅ Low error rate - within acceptable limits'
      }
    });
